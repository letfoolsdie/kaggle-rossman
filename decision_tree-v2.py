...
proInt = stores.PromoInterval.apply(myPinterval).tolist()
proInt = pd.DataFrame(proInt, columns = ['ProInt'+ str(i) for i in range(1,13)])
stores = stores.drop('PromoInterval',1).join(proInt)

#Changing instead to dummy variables:
df['StateHoliday'] = df.StateHoliday.astype(str)
df = df.drop('StateHoliday',1).join(pd.get_dummies(df['StateHoliday']).rename(columns=lambda x: 'StateHoliday' +"_"+str(x)))
...
stores = stores.drop('StoreType',1).join(pd.get_dummies(stores['StoreType']).rename(columns=lambda x: 'StoreType' +"_"+str(x)))  
stores = stores.drop('Assortment',1).join(pd.get_dummies(stores['Assortment']).rename(columns=lambda x: 'Assortment' +"_"+str(x)))  
...
test[[ 'Id', 'Sales' ]].to_csv('rand_for_v3-0-1.csv', index = False )
